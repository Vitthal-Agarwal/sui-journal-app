...
export function JournalList({
  ...
  // Fetch all Journal objects owned by the current user
  const { data, isPending, error } = useSuiClientQuery(
    ...
    {
      // Only enable when we have an account address
  ...
  if (error) {
    return (
      <Text color="red">Error loading journals: {(error as any).message}</Text>
    );
  }

  const journals = (data as any)?.data || [];
